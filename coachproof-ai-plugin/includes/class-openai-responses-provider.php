...
class OpenAI_Responses_Provider implements Chat_Provider_Interface {
    ...
    public function send_message( string $message, array $context = array() ): Chat_Result {
        ...
        if ( empty( $api_key ) ) {
            error_log( "[coachproof-ai][{$trace_id}] ERROR: No API key configured." );
            return $this->error_result( 'provider_config_error', $trace_id );
        }

        $model          = get_option( 'coachproof_chatbot_model', 'gpt-4o' );
        $system_prompt  = get_option( 'coachproof_chatbot_system_prompt', 'You are a professional coaching assistant. Guide the user towards the most appropriate coaching module based on their needs.' );
        $timeout        = (int) get_option( 'coachproof_chatbot_timeout', 30 );
        ...
        if ( ! is_array( $data ) ) {
            error_log( "[coachproof-ai][{$trace_id}] Malformed JSON response from OpenAI." );
        ...
        if ( empty( $reply_text ) ) {
            error_log( "[coachproof-ai][{$trace_id}] Empty reply_text in OpenAI response." );
     ...
     * 2. coachproof_chatbot_api_key option (admin settings fallback)
     *
     * @return string
     */
